...
export class PostTimelineAggregate extends Aggregate {
  ...
  constructor(organizationId: string) {
    ...
    this.registerHandler<PostScheduledPayload>(
      POST_EVENT_TYPES.POST_SCHEDULED,
      this.onPostScheduled.bind(this)
    )

    this.registerHandler<PostUpdatedPayload>(
      POST_EVENT_TYPES.POST_UPDATED,
      this.onPostUpdated.bind(this)
    )

    this.registerHandler<PostCancelledPayload>(
      POST_EVENT_TYPES.POST_CANCELLED,
      this.onPostCancelled.bind(this)
    )
  }
   ...
   */
  cancelPost(params: { postId: string; reason: string }): void {
    const post = this.posts.get(params.postId)
    ...
    // Raise event
    const payload: PostCancelledPayload = {
      postId: params.postId,
  ...
  private onPostScheduled(payload: PostScheduledPayload): void {
  ...
  private onPostUpdated(payload: PostUpdatedPayload): void {
  ...
  private onPostCancelled(payload: PostCancelledPayload): void {
    const post = this.posts.get(payload.postId)
    if (post) {
