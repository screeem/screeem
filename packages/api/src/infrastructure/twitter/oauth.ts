 ...
 */
export async function generateAuthUrl(state: TwitterOAuthState): Promise<string> {
...
export async function handleCallback(
  ...
  // Decode state
  const stateData: TwitterOAuthState = JSON.parse(
 ...
 */
export async function refreshAccessToken(encryptedRefreshToken: string): Promise<TwitterTokens> {
  if (!env.TWITTER_CLIENT_ID || !env.TWITTER_CLIENT_SECRET) {
    throw new Error('Twitter OAuth not configured')
  }
