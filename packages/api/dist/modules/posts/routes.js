...
import { schedulePostSchema, updatePostSchema, cancelPostSchema, } from '@screeem/shared';
import { checkUserOrgAccess } from '../../db/organizations.js';
import { handleSchedulePost, handleUpdatePost, handleCancelPost, } from '../../domain/commands/post-commands.js';
export async function registerPostRoutes(fastify) {
    /**
     * Schedule a new post (Command)
     * POST /api/organizations/:id/posts
     */
    fastify.post('/api/organizations/:id/posts', {
     ...
     */
    fastify.patch('/api/organizations/:id/posts/:postId', {
        onRequest: [fastify.authenticate],
    }, async (request, reply) => {
        const { id: organizationId, postId } = request.params;
        const userId = request.supabaseUser.id;
        // Check access
        const hasAccess = await checkUserOrgAccess(pool, organizationId, userId);
        if (!hasAccess) {
     ...
     */
    fastify.delete('/api/organizations/:id/posts/:postId', {
        onRequest: [fastify.authenticate],
    }, async (request, reply) => {
        const { id: organizationId, postId } = request.params;
        const userId = request.supabaseUser.id;
     ...
     */
    fastify.get('/api/organizations/:id/posts', {
        onRequest: [fastify.authenticate],
    }, async (request, reply) => {
        const { id: organizationId } = request.params;
        const userId = request.supabaseUser.id;
     ...
     */
    fastify.get('/api/organizations/:id/posts/:postId', {
        onRequest: [fastify.authenticate],
    }, async (request, reply) => {
        const { id: organizationId, postId } = request.params;
        const userId = request.supabaseUser.id;
        ...
        const result = await pool.query(`SELECT sp.*, u.display_name as created_by_name
         FROM scheduled_posts sp
         JOIN users u ON sp.created_by = u.id
         WHERE sp.id = $1 AND sp.organization_id = $2`, [postId, organizationId]);
