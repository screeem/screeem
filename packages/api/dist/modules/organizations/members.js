...
import { getOrganizationById, getOrganizationMembers, getOrganizationMember, addOrganizationMember, updateMemberRole, removeOrganizationMember, checkUserOrgAccess, } from '../../db/organizations.js';
export async function registerMemberRoutes(fastify) {
    ...
    // Update member role
    fastify.patch('/api/organizations/:id/members/:userId', {
        onRequest: [fastify.authenticate],
    }, async (request, reply) => {
    ...
    fastify.delete('/api/organizations/:id/members/:userId', {
        onRequest: [fastify.authenticate],
    }, async (request, reply) => {
        const { id, userId: targetUserId } = request.params;
        const userId = request.supabaseUser.id;
