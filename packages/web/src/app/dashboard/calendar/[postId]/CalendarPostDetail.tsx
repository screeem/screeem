...
export function CalendarPostDetail({
  ...
  if (loading) return <p className="py-16 text-center text-sm text-muted-foreground">Loading post…</p>
  if (!draft) return <div className="rounded-xl border border-border bg-card p-8 text-center"><h1 className="text-xl font-semibold">Post not found</h1><p className="mt-2 text-sm text-muted-foreground">This post does not exist or is no longer active.</p><Link href="/dashboard/calendar" className="mt-5 inline-block text-sm font-semibold text-primary">Back to calendar</Link></div>
      ...
      <div className="space-y-6">
        <section className="rounded-2xl border border-border bg-card p-6">
          ...
          <div className="mt-5 space-y-5">
            <label className="block text-xs font-semibold text-muted-foreground">Title<input value={draft.title} onChange={(event) => setDraft({ ...draft, title: event.target.value })} className="mt-2 w-full rounded-lg border border-border px-3 py-2.5 text-sm font-normal outline-none focus:border-primary focus:ring-2 focus:ring-ring/30" /></label>
            <label className="block text-xs font-semibold text-muted-foreground">Post copy<textarea value={draft.copy} onChange={(event) => setDraft({ ...draft, copy: event.target.value })} rows={7} className="mt-2 w-full resize-y rounded-lg border border-border px-3 py-2.5 text-sm font-normal leading-6 outline-none focus:border-primary focus:ring-2 focus:ring-ring/30" /><span className="mt-1 block text-right font-normal text-muted-foreground">{draft.copy.length} characters</span></label>
            ...
            <fieldset><legend className="text-xs font-semibold text-muted-foreground">Publish to</legend><div className="mt-2 flex flex-wrap gap-2">{targets.map((target) => <button type="button" key={target} onClick={() => toggleTarget(target)} className={`rounded-full border px-3 py-2 text-xs font-medium ${draft.targets.includes(target) ? "border-primary bg-primary-subtle text-primary" : "border-border text-muted-foreground"}`}>{target}</button>)}</div></fieldset>
...
function ApprovalPanel({
  ...
  return <section className="rounded-2xl border border-border bg-card p-6">
    <div className="flex items-start justify-between gap-4">
      <div><h2 className="text-base font-semibold">Approval</h2><p className="mt-1 text-xs text-muted-foreground">Decisions apply only to revision {post.revision}.</p></div>
      <ApprovalBadge status={approval.status} />
    </div>
    {approval.comment ? <blockquote className="mt-4 rounded-lg bg-muted px-4 py-3 text-sm text-muted-foreground">{approval.comment}</blockquote> : null}
    {dirty ? <p className="mt-4 rounded-lg bg-warning-subtle px-4 py-3 text-xs text-warning-text">Save your edits before changing approval. Saving creates a new revision and returns the post to Draft.</p> : null}
    <label className="mt-4 block text-xs font-semibold text-muted-foreground">Review note<textarea value={comment} maxLength={2000} onChange={(event) => onComment(event.target.value)} rows={3} placeholder={approval.status === "in_review" ? "Add context for the decision" : "Add context for reviewers"} className="mt-2 w-full resize-y rounded-lg border border-border px-3 py-2.5 text-sm font-normal outline-none focus:border-primary" /></label>
    <div className="mt-4 flex flex-wrap gap-2">
      {(approval.status === "draft" || approval.status === "changes_requested") ? <button disabled={busy || dirty} onClick={() => void onAction("approval.requested")} className="rounded-lg bg-primary px-3.5 py-2 text-sm font-semibold text-primary-foreground disabled:opacity-40">Request approval</button> : null}
      {approval.status === "in_review" && canApprove ? <button disabled={busy || dirty} onClick={() => void onAction("approval.granted")} className="rounded-lg bg-success px-3.5 py-2 text-sm font-semibold text-primary-foreground disabled:opacity-40">Approve revision</button> : null}
      {approval.status === "in_review" && canApprove ? <button disabled={busy || dirty || !comment.trim()} onClick={() => void onAction("approval.changes_requested")} className="rounded-lg border border-warning px-3.5 py-2 text-sm font-semibold text-warning-text disabled:opacity-40">Request changes</button> : null}
      {approval.status === "in_review" && canWithdraw ? <button disabled={busy || dirty} onClick={() => void onAction("approval.withdrawn")} className="rounded-lg border border-border px-3.5 py-2 text-sm font-semibold text-muted-foreground disabled:opacity-40">Withdraw</button> : null}
    </div>
    {approval.status === "in_review" && !canApprove ? <p className="mt-3 text-xs text-muted-foreground">A team owner or admin must review this revision.</p> : null}
  </section>
}
...
function SocialPreviews({ post }: { post: CalendarPost }) {
  return <aside className="space-y-4 lg:sticky lg:top-6 lg:self-start"><div><h2 className="text-base font-semibold">Social previews</h2><p className="mt-1 text-xs text-muted-foreground">Live preview for each selected network.</p></div>{post.targets.map((target) => <PreviewCard key={target} target={target} copy={post.copy} />)}{post.targets.length === 0 ? <div className="rounded-xl border border-dashed border-border p-8 text-center text-sm text-muted-foreground">Select a social network to see its preview.</div> : null}</aside>
}

function PreviewCard({ target, copy }: { target: CalendarTarget; copy: string }) {
  const name = target === "X" ? "Screeem" : "Screeem Studio"
  return <section className="overflow-hidden rounded-2xl border border-border bg-card"><div className="flex items-center justify-between border-b border-border px-4 py-3"><span className="text-xs font-semibold text-muted-foreground">{target} preview</span><span className="text-xs text-muted-foreground">{copy.length} chars</span></div><div className={target === "Instagram" ? "p-0" : "p-5"}>{target === "Instagram" ? <div className="aspect-square bg-gradient-to-br from-platform-instagram-from via-platform-instagram-via to-platform-instagram-to p-8 text-platform-instagram-foreground"><div className="flex h-full items-center justify-center rounded-2xl border border-platform-instagram-foreground/30 bg-platform-instagram-foreground/10 p-6 text-center text-xl font-semibold leading-snug backdrop-blur-sm">{copy || "Your post preview"}</div></div> : <><div className="flex gap-3"><div className={`grid size-10 shrink-0 place-items-center rounded-full font-bold ${target === "X" ? "bg-platform-x text-platform-x-foreground" : "bg-platform-linkedin text-platform-linkedin-foreground"}`}>S</div><div className="min-w-0"><p className="text-sm font-semibold text-foreground">{name} <span className="font-normal text-muted-foreground">{target === "X" ? "@screeem · now" : "1st · now"}</span></p><p className="mt-2 whitespace-pre-wrap break-words text-sm leading-6 text-foreground">{copy || "Your post copy will appear here."}</p></div></div><div className="mt-4 flex justify-between border-t border-border pt-3 text-xs text-muted-foreground"><span>♡ Like</span><span>↗ Share</span><span>◯ Comment</span></div></>}</div>{target === "Instagram" ? <p className="px-4 py-3 text-sm text-foreground"><strong>screeem</strong> {copy || "Your caption will appear here."}</p> : null}</section>
}
