...
import { createFormDefinition } from "@screeem/forms"
...
export async function GET(_request: NextRequest, context: { params: Promise<{ teamId: string }> }) {
  ...
  const { data, error } = await admin
    .from("forms")
    .select(
      "id, name, endpoint_key, allowed_origin, success_url, is_active, requires_turnstile, submission_schema, legacy_unstructured, availability:definition_availability, draft_revision, published_version, created_at",
    )
    .eq("team_id", teamId)
...
export async function POST(request: NextRequest, context: { params: Promise<{ teamId: string }> }) {
  ...
  if (!name || name.length > 80)
    return NextResponse.json(
      { error: "Form name must be between 1 and 80 characters" },
      { status: 400 },
    )
  try {
    const admin = createAdminClient()
    const { data, error } = await admin
      ...
      .insert({
        ...
        draft_definition: createFormDefinition(name),
      })
      .select(
        "id, name, endpoint_key, allowed_origin, success_url, is_active, requires_turnstile, submission_schema, legacy_unstructured, availability:definition_availability, draft_revision, published_version, created_at",
      )
