...
function renderTweetText(text: string): string {
  // Tokenize URLs, @mentions, and #hashtags on raw text BEFORE escaping,
  // so that HTML entities like &#039; don't get matched as hashtags.
  ...
  // Order matters: URLs first (so http://…#foo isn't split), then @mentions, then #hashtags
  let processed = text;
  processed = tokenize(processed, /(https?:\/\/[^\s]+)/g, (_, url) =>
    `<a href="${escapeHtml(url)}">${escapeHtml(url)}</a>`
  );
  processed = tokenize(processed, /@(\w+)/g, (_, handle) =>
    `<a href="#">@${escapeHtml(handle)}</a>`
  );
  processed = tokenize(processed, /#(\w+)/g, (_, tag) =>
    `<a href="#">#${escapeHtml(tag)}</a>`
  );

  // Now HTML-escape everything that isn't a token
  let html = escapeHtml(processed);

  // Restore tokens (they were escaped too, but \x00 isn't affected by escapeHtml)
...
function renderTweet(tweet: TweetData): string {
  ...
  return `
    <div class="tweet-card">
      ...
      <div class="tweet-text">${renderTweetText(tweet.text)}</div>
    ...
    <div class="char-count${isOverLimit ? " over-limit" : ""}">
      <svg width="32" height="32" viewBox="0 0 32 32">
        <circle cx="16" cy="16" r="14" fill="none" stroke-width="2.5" stroke="${document.body.classList.contains("dark") ? "#2f3336" : "#e1e8ed"}"></circle>
...
const app = new App({ name: "Tweet Preview", version: "0.0.1" });
...
app.ontoolresult = (params) => {
  ...
  try {
    const tweet: TweetData = JSON.parse(text.text);
    root.innerHTML = renderTweet(tweet);
  } catch {
...
app.onhostcontextchanged = (params) => {
  if (params.theme) {
    ...
    // Re-render to update SVG stroke colors if we have tweet data
    const existingCard = root.querySelector(".tweet-card");
    if (existingCard) {
      // The card is already rendered, just update theme class
    }
  }
};
